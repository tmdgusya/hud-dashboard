...
import { emitHeartbeat } from "./eventBus.js";
...
export function setupHUDWidget(ctx: any): void {
  ...
  const updateWidget = (state: any) => {
    if (!currentRenderer) return;
    const dashboard = currentRenderer.render(state);
    ctx.ui.setWidget("hud-dashboard", () => dashboard);
  };

  unsubscribe = hudStore.subscribe(updateWidget);
  updateWidget(hudStore.getState());
}
