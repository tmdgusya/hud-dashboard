...
import { widgetRegistry, registerHUDCommands, setHUDTheme, initHUD, destroyHUD, setupHUDWidget } from "./ui.js";
...
export default function hudDashboardExtension(pi: ExtensionAPI): void {
  ...
  pi.on("session_start", async (_event, ctx) => {
    if (ctx.hasUI) {
      ...
      ctx.ui.notify("Try /hud-metrics, /hud-tools, /hud-minimize", "info");
      ctx.ui.setStatus("hud", "HUD Ready");
    }
  });

  // State sync & Dashboard setup
  initHUD(pi);
  initHUDDashboard(pi);
